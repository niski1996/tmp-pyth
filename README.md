...
- 🎯 Entry points to execute your program using `python -m <tmp_pyth>` or `$ tmp_pyth` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/niski1996/tmp-pyth/branch/main/graph/badge.svg?token=tmp-pyth_token_here)](https://codecov.io/gh/niski1996/tmp-pyth)
[![CI](https://github.com/niski1996/tmp-pyth/actions/workflows/main.yml/badge.svg)](https://github.com/niski1996/tmp-pyth/actions/workflows/main.yml)

Awesome tmp_pyth created by niski1996

## Install

from source
```bash
git clone https://github.com/niski1996/tmp-pyth tmp_pyth
cd tmp_pyth
make install
```
...
```bash
pip install tmp_pyth
```

## Executing

```bash
$ tmp_pyth run --port 8080
```

or

```bash
python -m tmp_pyth run --port 8080
```

or

```bash
$ uvicorn tmp_pyth:app
```
...
```bash
❯ tmp_pyth --help
Usage: tmp_pyth [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ tmp_pyth create-user --help
Usage: tmp_pyth create-user [OPTIONS] USERNAME PASSWORD
...
```bash
tmp_pyth create-user admin admin --superuser
```
...
Run with `tmp_pyth run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `tmp_pyth create-user`.
...
-----------------------------------------------------
tmp_pyth/__init__.py              4      0   100%
tmp_pyth/app.py                  16      1    94%
tmp_pyth/cli.py                  21      0   100%
tmp_pyth/config.py                5      0   100%
tmp_pyth/db.py                   10      0   100%
tmp_pyth/models/__init__.py       0      0   100%
tmp_pyth/models/content.py       47      1    98%
tmp_pyth/routes/__init__.py      11      0   100%
tmp_pyth/routes/content.py       52     25    52%
tmp_pyth/routes/security.py      15      1    93%
tmp_pyth/routes/user.py          52     26    50%
tmp_pyth/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in tmp_pyth/default.toml.
